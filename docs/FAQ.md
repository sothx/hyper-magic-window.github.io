...
### 6. 为什么有些应用打开是首页全屏，有些应用打开是默认分屏，而有些应用打开是默认只有中间是内容区域，两边空白？

给应用适配是一件工作量非常大的事情，平均人工精修一个应用就要耗费十几二十分钟，如果应用复杂程度比较高，耗时还会长达一两个小时。

因为人工精修一个应用成本极大，还要考虑到各个应用的可适配质量参差不齐，因此只给了少部分应用做了首页全屏化、首页分屏化，页面逻辑递进等精细化的适配逻辑，而且这些应用适配都是经过人工逐一适配得出的最佳方案。

模块补齐的大多数应用更多是首页只有中间是内容区域，点击内容后展开二级页面的设计（别家平板也基本是这样的体验），如果有哪些应用首页全屏化体验更好，或者首页默认分屏的体验更好，或者有比较合适的页面递进逻辑，可以反馈给我，也欢迎贡献应用的适配规则。
