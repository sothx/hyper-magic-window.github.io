...
模块目前总适配应用数量接近8000+，毫不夸张地说，安装《完美横屏应用计划》之后，对于平行视界的适配数量将仅次于华为 MatePad 系列，并且我们针对其中高频使用的700+应用手工做了精适配。
...
维护这个模块只是因为我自己也是在用小米平板，因此仅对 MIUI 或 Hyper OS 做适配。

如果你对其他设备的模块支持有想法，欢迎通过Github贡献相关代码。
...
### 13. 为什么我的滑动条不能左右调整比例？我的是Mix Fold 系列。

官方不支持，我也没办法 + 2。

### 14. 为什么有时候过渡动画很奇怪，整个窗口旋转了一圈，有时候又会黑一下？
...
### 17.可以像 MatePad 那样固定左边/右边的页面么或者两边显示一样的同级页面，以及小窗模式下也有平行视界？
...
新版京东已经开始自己做平板适配，请确保你的京东已经升级到最新版，首次打开京东的界面还未触发平板判断，第二次打开方可正常使用。
...
### 6.智能助理添加的快递小组件，点击进去淘宝只有右侧界面，而且会卡死，怎么解决？

新版淘宝已经自适配平板，模块不再对淘宝的适配施加任何干预，这个问题是由于淘宝的平板适配缺陷产生的，请多多反馈给淘宝官方去修复。

### 7.为什么哔哩哔哩看直播无法全屏？还有一些应用界面也显示不全？
...
### 10.为什么QQ/微信视频通话，对方显示的画面旋转90度？

跟模块没关系，QQ/微信一直以来的App适配设计问题，非系统问题，请向对应App的开发者反馈此问题。

### 11.为什么闲鱼的适配不是默认分屏？官方的适配是默认分屏。

闲鱼默认展开分屏后会出现页面崩溃的BUG，任何时候模块的适配都是实用性为主，因此取消了闲鱼的默认分屏。

### 12.CCTalk为什么没有适配？我看小米应用商店的页面是有适配的。

CCTalk自己并没有做好平板适配，导致进入视频播放页没办法正常退出，加上CCTalk自己遗留的屎山代码，导致CCTalk也无非正常被系统反向适配，因此如果需要CCTalk的平行窗口，需要升级模块到至少1.23.88，并且安装提供的修改版CCTalk客户端。

修改版CCTalk安装包(无签名，需搭配核心破解食用，也可以使用HyperCeiler内置的核心破解)
