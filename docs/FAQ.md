...
之前折叠屏的适配随着QQ音乐的更新存在BUG，而且也并不是很好的适配方案，产生了比较多的副作用，因此移除了QQ音乐的平行视界适配，可以通过模块V13版本提供的[自定义规则](https://hyper-magic-window.sothx.com/custom-config.html)将QQ音乐的适配调整为老的平行视界适配规则。
